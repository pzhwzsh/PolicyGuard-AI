...
- Complete verification now collects 99 tests. Locally, 98 pass and the PostgreSQL-only migration test is conditionally skipped; GitHub CI provisions PostgreSQL 16 to run that final case. Whole-package statement coverage is 69%; CLI and worker entry points remain the largest uncovered area. Latest management UI was checked at desktop and 390px mobile width with no horizontal overflow or console errors.
...
- RapidOCR smoke verification produced one canonical block with the exact text `Advertising must be truthful`, page/bbox/hash metadata, and confidence 0.9963.
...
- Auditable data release under `data/evidence/v1`: 11 latest official-source copies/3,439 parsed sections, source and release hashes, dataset inventory, sanitized model metrics, and an honest runtime snapshot. See `docs/DATA_CARD.md`; staged sources remain pending legal review and are not silently added to active RAG.
