...
> 最后核对：2026-07-22
>
> 当前版本：`main@e89c8b5`（本文更新前）
>
> 项目阶段：本地面试级工程项目；暂不部署公网
...
| 自动化测试 | 收集 102 项；本地 101 通过，PostgreSQL 用例无连接时跳过 |
...
- 批量 CSV/XLSX 商品审核和可恢复任务。
- 图片、视频广告中的 OCR、字幕、视觉声明和帧级引用。
- 法规变化影响分析：定位受影响商品、历史报告、缓存和 Agent 记忆。
...
2. 再做批量 XLSX 审核，将项目与真实商品审核业务连接起来。
3. 然后做法规变化影响分析，证明版本化知识库、记忆失效和历史报告能形成闭环。
4. 最后补图片/视频多模态审核；公网部署仍等用户明确授权。
